...
class MrpWorkcenter(orm.Model):
    ...
    _columns = {
        'work_hour': fields.float(
            'Work hour', digits=(10, 2),
            help='Normal work hour a day'),
        'extra_work_hour': fields.float(
            'Extra work hour', digits=(10, 2),
...
class ProductTemplateAccounting(orm.Model):
    ...
    _columns = {
        'minimum_qty': fields.float(
            'Min. quantity', digits=(10, 2),
            help="Minimum value for stock"),
        'maximum_qty': fields.float(
            'Max. quantity', digits=(10, 2),
            help="Maximum value for stock"),
        'accounting_qty': fields.float(
            'Accounting quantity', digits=(10, 2),
        ...
        for order in self.browse(cr, uid, ids, context=context):
            is_line_produced = [
                item.product_uom_qty <= (
                    item.product_uom_maked_sync_qty +
...
class SaleOrderLineRelation(orm.Model):
    ...
    def _mrp_function_similar(self, cr, uid, ids, fields, args, context=None):
        ...
        mrp_ids = mrp_pool.search(cr, uid, [
            ('state', 'not in', ('done', 'cancel')),  # TODO only open jobs!
            ], context=context)
...
class SaleOrderLine(orm.Model):
    ...
    def _mrp_get_default_code_from_product(self, cr, uid, ids, context=None):
        ''' Change default code in product
        '''
...
class ResPartner(orm.Model):
    ...
    _columns = {
        ...
        'print_in_cut': fields.boolean('Report taglio'),
        }
