    ...
    def default_oc_list(self, cr, uid, field, context=None):
        ...
        default = {
            "list": _("- Q. in store (don't produce): %6.3f\n") % (
                sol_browse[0].product_id.accounting_qty if (
                    len(sol_browse)>0 and sol_browse[0].product_id) else 0.0), 
            "error": False, 
            "total": 0.0 - (
                sol_browse[0].product_id.accounting_qty if (
                    len(sol_browse)>0 and sol_browse[0].product_id) else 0.0), 
        ...
        try:
            if field=="bom":
                return self.pool.get("mrp.bom").search(cr, uid, [
                    ('product_id','=',sol_browse[0].product_id.id),
                    ], context=context)
        except:
            return False    
            ...
            if field == "list":
                ...
                res+="+ OC: %s [Row: %s] q.: %s (Scad.: %s).\n" % (
                    ...
                    "%s/%s/%s" % (
                        ...
                        ) if item.date_deadline else "Not present!"
                    )
            elif field == "error":
                if item.product_id.id != old_product_id:
                    return True
